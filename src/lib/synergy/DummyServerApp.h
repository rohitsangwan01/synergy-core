...
class DummyServerApp : public App {
public:
  DummyServerApp(IEventQueue *events, ClientProxy *client);
  virtual ~DummyServerApp();
  ...
  void closeClientListener(DummyClientListener *listen);
  void stopServer();
  ...
  DummyClientListener *openClientListener(const NetworkAddress &address);
  ...
  static DummyServerApp &instance() {
    return (DummyServerApp &)App::instance();
  }
  ...
  DummyClientListener *m_listener;
