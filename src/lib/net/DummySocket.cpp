...
DummySocket::DummySocket(IEventQueue *events,
...
void DummySocket::bind(const NetworkAddress &addr) {
  try {
...
void *DummySocket::getEventTarget() const {
  return const_cast<void *>(static_cast<const void *>(this));
}

UInt32 DummySocket::read(void *buffer, UInt32 n) {
...
void DummySocket::write(const void *buffer, UInt32 n) {
  bool wasEmpty;
  {
    Lock lock(&m_mutex);
...
void DummySocket::shutdownInput() {
...
void DummySocket::shutdownOutput() {
  bool useNewJob = false;
  {
    Lock lock(&m_mutex);
...
bool DummySocket::isReady() const {
  Lock lock(&m_mutex);
  return (m_inputBuffer.getSize() > 0);
}

bool DummySocket::isFatal() const {
  // TCP sockets aren't ever left in a fatal state.
  LOG((CLOG_ERR "isFatal() not valid for non-secure connections"));
  return false;
}

UInt32 DummySocket::getSize() const {
  Lock lock(&m_mutex);
  return m_inputBuffer.getSize();
}

void DummySocket::connect(const NetworkAddress &addr) {
  {
    Lock lock(&m_mutex);
...
DummySocket::EJobResult DummySocket::doRead() {
  UInt8 buffer[4096];
...
DummySocket::EJobResult DummySocket::doWrite() {
  // write data
  UInt32 bufferSize = 0;
  int bytesWrote = 0;
...
void DummySocket::setJob(ISocketMultiplexerJob *job) {
...
ISocketMultiplexerJob *DummySocket::newJob() {
  ...
  if (m_socket == nullptr) {
    ...
    return new TSocketMultiplexerMethodJob<DummySocket>(
        this, &DummySocket::serviceConnecting, m_socket, m_readable,
        m_writable);
  } else {
    ...
    return new TSocketMultiplexerMethodJob<DummySocket>(
        this, &DummySocket::serviceConnected, m_socket, m_readable,
        m_writable && (m_outputBuffer.getSize() > 0));
  }
}

void DummySocket::sendConnectionFailedEvent(const char *msg) {
...
void DummySocket::sendEvent(Event::Type type) {
  m_events->addEvent(Event(type, getEventTarget()));
}

void DummySocket::discardWrittenData(int bytesWrote) {
...
void DummySocket::onConnected() {
  m_connected = true;
  m_readable = true;
  m_writable = true;
}

void DummySocket::onInputShutdown() {
  m_inputBuffer.pop(m_inputBuffer.getSize());
  m_readable = false;
}

void DummySocket::onOutputShutdown() {
...
void DummySocket::onDisconnected() {
  // disconnected
  onInputShutdown();
  onOutputShutdown();
  m_connected = false;
...
DummySocket::serviceConnecting(ISocketMultiplexerJob *job, bool, bool write,
                               bool error) {
  Lock lock(&m_mutex);
...
ISocketMultiplexerJob *DummySocket::serviceConnected(ISocketMultiplexerJob *job,
